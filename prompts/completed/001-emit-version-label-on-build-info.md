---
status: completed
summary: Replaced private metrics package with github.com/bborbe/metrics, added BuildGitVersion field, and wired SetBuildInfo with version, commit, and date
execution_id: alert-controller-exec-001-emit-version-label-on-build-info
dark-factory-version: v0.192.9
created: "2026-07-25T08:34:00Z"
queued: "2026-07-25T08:34:00Z"
started: "2026-07-25T08:38:56Z"
completed: "2026-07-25T08:42:09Z"
---
...
2. In `main.go`, change the import alias `libmetrics` to point at the shared library instead of the local package:
   - remove `libmetrics "github.com/bborbe/alert-controller/pkg/metrics"`
...
4. Delete the private copy entirely: remove the file `pkg/metrics/build-info-metrics.go`. It is the only file in `pkg/metrics/`, so remove that now-empty directory too. Delete ONLY the `pkg/metrics/` directory and its contents — do not touch any other file under `pkg/`.
...
- This change is exempt from the Definition-of-Done testing criterion: `main.go` has no test harness, and the behavior being wired in is covered by the upstream library's own tests (`metrics_build_info_test.go`). The exemption is a property of this change, not something to work around.
- `README.md` has no configuration table, and the build-metadata variables (`BUILD_GIT_COMMIT`, `BUILD_DATE`) are already undocumented there. Do not add one and do not document `BUILD_GIT_VERSION` — README needs no change for this prompt.
</constraints>
...
- `grep -r "alert-controller/pkg/metrics" --include="*.go" .` returns no matches outside `vendor/`
